                ...
                $.ajax({  
                    ...
                    success:function(data){
                        if (data == 'Username sudah digunakan' || data == 'Konfirmasi password salah' || data == 'Gambar gagal di upload' || data == 'Harap pilih barang dahulu') {
                            ...
                            if ($('#input_url').val() == "<?php echo base_url('petugas/lelang_barang/input_barang') ?>") {
                                window.location.assign("<?php echo base_url('petugas/lelang_barang/index')?>");
                            }else if ($('#input_url').val() == "<?php echo base_url('petugas/lelang_lelang/input_lelang') ?>") {
                                window.location.assign("<?php echo base_url('petugas/lelang_lelang/index')?>");
                            }else{
                                location.reload();
                            }
                        }   
    ...
    <script >
        $('#tabel_ptg').on('click','.item_edit',function(){
            ...
            $.ajax({
                type:"GET",
                url:"<?php echo base_url('petugas/data/tampil_data_ubah_petugas')?>",
                dataType:"JSON",
                data:{id:id}, 
                success:function(data){
    ...
    <script>
        $('#tabel_msy').on('click','.item_edit',function(){
            ...
            $.ajax({
                type:"GET",
                url:"<?php echo base_url('petugas/data/tampil_data_ubah_masyarakat')?>",
                dataType:"JSON",
                data:{id:id}, 
                success:function(data){
                ...
                $.ajax({  
                    ...
                    success:function(data){
                        if (data == 'Username sudah digunakan' || data == 'Harap isi kolom password baru jika ingin mengubah password, jika tidak kosongkan password lama dan password baru' || data == 'Password lama salah' || data == 'Gambar gagal di upload' ) {
                            ...
                            if ($('#update_url').val() == "<?php echo base_url('petugas/lelang_barang/update_barang') ?>") {
                                window.location.assign("<?php echo base_url('petugas/lelang_barang/index')?>");
                            }else if ($('#update_url').val() == "<?php echo base_url('petugas/lelang_lelang/update_lelang') ?>") {
                                window.location.assign("<?php echo base_url('petugas/lelang_lelang/index')?>");
                            }else{
                                location.reload();
                            }
    ...
    <script>
        $(document).ready(function(){
            ...
            {
                $.ajax({
                    url:"<?php echo base_url('petugas/lelang_lelang/detail_tambah_lelang'); ?>",
                    method:"POST",
                    data:{query:query},
                    success:function(data){
