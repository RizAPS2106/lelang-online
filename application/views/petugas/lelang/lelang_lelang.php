<div class="container-fluid">
	...
	<?php echo anchor('petugas/lelang_lelang/tambah_lelang','<button class="btn btn-sm btn-warning mb-3"><i class="fas fa-fw fa-sm fa-plus"></i> Buka Lelang</button>') ?>
	...
	<table class="table table-dark table-borderless table-striped" id="example">
		...
		<tr>
			...
			<td>
				<?php echo anchor('petugas/lelang_lelang/detail_lelang/'.$llg->id_lelang,'<div class="btn btn-sm btn-success"><i class="fas fa-search-plus"></i></div>') ?>
			<?php if ($llg->status == "dibuka") { ?>
				<a href="<?php echo base_url('petugas/lelang_lelang/ubah_lelang/'.$llg->id_lelang) ?>"><div class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></div></a>
			<?php }else if ($llg->status == "ditutup") { ?>
				<a href="" onclick="javascript: alert('Pelelangan sudah ditutup')"><div class="btn btn-sm btn-secondary"><i class="fa fa-edit"></i></div></a>
			<?php } ?>
				<a href="<?php echo base_url('petugas/lelang_lelang/hapus_lelang/'.$llg->id_lelang) ?>" onclick="javascript: return confirm('Yakin akan di hapus?')"><div class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></div></a>
			</td>
		</tr>	
		<?php 
			endforeach; 
